        ...
        <div className={wrapperClass}>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50 flex-shrink-0">
                <div className={`items-center gap-3 flex-wrap ${isFullscreen ? 'hidden sm:flex' : 'flex'}`}>
                    ...
                    <span className="flex items-center gap-1.5 px-2.5 py-1 bg-red-50 border border-red-200 rounded-full text-xs font-semibold text-red-700">
                        ...
                        {awayTeamName || t('tacticalBoard.opponent')} — {state.opponents.length} {t('tacticalBoard.players')}
                    </span>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${isHomeGame ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                        {isHomeGame ? t('tacticalBoard.home') : t('tacticalBoard.away')}
                    </span>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-slate-400 hidden sm:block">{t('tacticalBoard.dragToReposition')}</p>
                    <button
                        onClick={flipSides}
                        title={t('tacticalBoard.flipSides')}
                    ...
                    >
                        ...
                        <span className="hidden sm:inline">{t('tacticalBoard.flipSides')}</span>
                    </button>
                    <button
                        onClick={() => setIsFullscreen(f => !f)}
                        title={isFullscreen ? t('tacticalBoard.exitFullscreen') : t('tacticalBoard.expandFullscreen')}
            ...
            <div className="flex flex-1 min-h-0" style={isFullscreen ? {} : { height: '520px' }}>
                ...
                <div className="flex flex-col items-center gap-2 p-2 border-r border-slate-100 bg-slate-50 flex-shrink-0">
                    ...
                    <ToolBtn active={false} onClick={undo} title={t('tacticalBoard.tool.undo')} disabled={!history.length}>
                        <RotateCcw className="w-3.5 h-3.5" />
                    </ToolBtn>
                    <ToolBtn active={false} onClick={() => { snapshot(); setState(s => ({ ...s, arrows: [] })); }} title={t('tacticalBoard.tool.clearArrows')}>
                ...
                <div className="flex-1 bg-slate-900 flex items-center justify-center overflow-hidden">
                    ...
                    >
                        ...
                        {(() => {
                            ...
                            const leftName  = ourOnLeft ? (homeTeamName || t('tacticalBoard.ourTeam')) : (awayTeamName || t('tacticalBoard.opponent'));
                            const rightName = ourOnLeft ? (awayTeamName || t('tacticalBoard.opponent')) : (homeTeamName || t('tacticalBoard.ourTeam'));
                            return (
                                <>
                    ...
                    <div className="border-b border-slate-100">
                        ...
                        <div className="overflow-y-auto p-2 space-y-1" style={{ maxHeight: '180px' }}>
                            ...
                            {state.ourPlayers.length === 0 && (
                                <p className="text-xs text-slate-400 text-center py-3 px-2">{t('tacticalBoard.noStarters')}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                        <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border-b border-slate-100 flex-shrink-0">
                            ...
                            <span className="text-xs font-bold text-red-700 uppercase tracking-wider">
                                {awayTeamName || t('tacticalBoard.opponent')}
                            </span>
                        </div>
                        ...
                        <div className="px-2 py-2 border-b border-slate-100 flex-shrink-0 space-y-1.5">
                            <input
                                ...
                                placeholder={t('tacticalBoard.playerName')}
                            ...
                            <div className="flex gap-1.5">
                                <input
                                    ...
                                    onKeyDown={e => e.key === 'Enter' && addOpponent()}
                                    placeholder={t('tacticalBoard.number')}
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            {state.opponents.map(op => (
                                <div key={op.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-red-50/60 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {op.number && <p className="text-[9px] text-slate-400">{t('tacticalBoard.number')} {op.number}</p>}
                                    </div>
                                    <button
                            ...
                            {state.opponents.length === 0 && (
                                <p className="text-xs text-slate-400 text-center py-4 px-2 leading-relaxed">
                                    {t('tacticalBoard.addOpponentHint')}
                                </p>
                            )}
                        </div>
                    </div>
