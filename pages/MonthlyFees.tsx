...
const MonthlyFees: React.FC = () => {
  ...
  const loadFees = async () => {
    try {
      ...
      setError(t('monthlyFees.errorLoading'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const getStatusBadge = (status: string) => {
    ...
    const labels = {
      paid: t('monthlyFees.status.paid'),
      pending: t('monthlyFees.status.pending'),
      overdue: t('monthlyFees.status.overdue'),
    };
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-sm font-medium text-slate-500 mb-1">{t('monthlyFees.totalExpected')}</p>
          <h3 className="text-xl font-bold text-slate-800">{formatCurrency(summary.totalExpected)}</h3>
          <p className="text-xs text-slate-400 mt-2">{fees.length} {t('monthlyFees.count')}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-slate-500 mb-1">{t('monthlyFees.received')}</p>
          ...
          <p className="text-xs text-slate-400 mt-2">
            {summary.totalExpected > 0 ? Math.round((summary.totalReceived / summary.totalExpected) * 100) : 0}% {t('monthlyFees.ofTotal')}
          </p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-slate-500 mb-1">{t('monthlyFees.status.pending')}</p>
          ...
          <p className="text-xs text-slate-400 mt-2">{summary.pendingCount} {t('monthlyFees.payments')}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-slate-500 mb-1">{t('monthlyFees.status.overdue')}</p>
          ...
          <p className="text-xs text-slate-400 mt-2">{summary.overdueCount} {t('monthlyFees.payments')}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              <option value="">{t('monthlyFees.allMonths')}</option>
              {uniqueMonths.map(m => (
          ...
          <div className="relative flex-1 min-w-[300px]">
            ...
            <input
              type="text"
              placeholder={t('monthlyFees.searchPlaceholder')}
              value={searchTerm}
        ...
        <div className="flex gap-2">
          ...
          >
            <Plus className="w-4 h-4" /> {t('monthlyFees.newFee')}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {t('monthlyFees.pendingFees')} ({summary.pendingCount})
        </button>
        <button
        ...
        >
          {t('monthlyFees.overdueFees')} ({summary.overdueCount})
        </button>
        <button
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{t('monthlyFees.athlete')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{t('athletes.category')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">{t('monthlyFees.dueDate')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">{t('monthlyFees.amount')}</th>
                      ...
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {fee.status !== 'paid' && (
                          <button
                            ...
                            title={t('monthlyFees.markAsPaid')}
                          >
        ...
        {fees.length > 0 && (
          <div className="px-6 py-3 border-t border-slate-100 text-sm text-slate-500">
            {filteredFees.length === fees.length ? (
              <span>{fees.length} {t('monthlyFees.count')}</span>
            ) : (
              <span>{filteredFees.length} {t('common.of')} {fees.length} {t('monthlyFees.count')}</span>
            )}
          </div>
        )}
      </div>
      ...
      <ConfirmModal
        ...
        title={t('monthlyFees.deleteTitle')}
        message={t('monthlyFees.deleteConfirm')}
      ...
      <ConfirmModal
        ...
        title={t('monthlyFees.confirmPaymentTitle')}
        message={t('monthlyFees.confirmPaymentMessage').replace('{name}', markPaidModal.fee?.athlete?.full_name || '')}
