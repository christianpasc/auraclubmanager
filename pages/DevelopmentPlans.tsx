  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Planos de Desenvolvimento</h1>
          <p className="text-slate-500 text-sm mt-0.5">PDIs individuais com metas por skill e prazo.</p>
        </div>
      ...
      {plans.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Ativos',     value: active,    color: 'text-indigo-600' },
            { label: 'Rascunhos',  value: draft,     color: 'text-slate-500'  },
            { label: 'Concluídos', value: completed, color: 'text-green-600'  },
          ].map(s => (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-slate-400">
          ...
          <p className="font-medium">{plans.length === 0 ? 'Nenhum PDI criado ainda' : 'Nenhum resultado encontrado'}</p>
          {plans.length === 0 && (
            <button onClick={() => navigate('/development-plans/new')}
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => setExpanded(e => e === plan.id ? null : plan.id!)}
                      ...
                      {expanded === plan.id ? 'Fechar' : 'Ver metas'}
                    </button>
                  </div>
                </div>
...
const GoalsExpanded: React.FC<{ planId: string }> = ({ planId }) => {
  ...
  if (loading) return <div className="px-5 pb-4 flex items-center gap-2 text-slate-400 text-sm"><Loader2 className="w-3.5 h-3.5 animate-spin"/> Carregando metas...</div>;
  if (!goals.length) return <div className="px-5 pb-4 text-sm text-slate-400">Nenhuma meta cadastrada.</div>;
